                            ...
                            <div class="col-md-7 mb-3 mb-md-0">
                                ...
                                <select class="form-select selectpicker" data-live-search="true" id="parent_select" name="id_parent" onchange="document.getElementById('parent_id').value = this.value;">
                                    ...
                                    {if isset($category_select)}
                                        {$incorrectParents = [(int)$category.id_cat]}

                                        {foreach $category_select as $item}
                                            {$i_parent = (int)$item.id_parent}
                                            {$i_cat  = (int)$item.id_cat}

                                            {if in_array($i_parent, $incorrectParents)}
                                                {if !in_array($i_cat, $incorrectParents)}{$incorrectParents[] = $i_cat}{/if}
                                            {elseif $i_cat != (int)$category.id_cat}
                                                <option value="{$i_cat}" {if ((int)$category.id_parent) == $i_cat}selected{/if}>
                                                    {$item.name_cat|default:'Catégorie sans nom'} (ID: {$i_cat})
                                                </option>
                                            {/if}
                                        {/foreach}
                                    {/if}
